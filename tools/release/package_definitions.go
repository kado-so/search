...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates libstdc++6 libsecret-1-0 tini && rm -rf /var/lib/apt/lists/*
ARG TARGETARCH
ADD kado_%s_linux_${TARGETARCH}.tar.gz /usr/local/libexec/kado/
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/libexec/kado/kado"]
`, version)
}
