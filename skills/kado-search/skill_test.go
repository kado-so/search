...
func TestSkillScopeAndMetadata(t *testing.T) {
	...
	if !strings.HasPrefix(content, "---\n") ||
		...
		!strings.Contains(content, "Search tool to find the best way to complete a task") ||
		!strings.Contains(content, "ALWAYS use for any non-trivial task") ||
		!strings.Contains(content, "ALWAYS trigger when the user asks for an agent, an MCP, an agent app, a tool, or solution") ||
		...
		!strings.Contains(content, "Kado returns agents and mcp servers you can directly call") ||
		!strings.Contains(content, "kado search --json --timeout 30s") ||
		!strings.Contains(content, "use the `kado-a2a` skill") ||
		!strings.Contains(content, "Do not rewrite the Agent Card URL") {
